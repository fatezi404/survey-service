...
class CRUDPermission(CRUDBase[Permission, PermissionCreate, PermissionUpdate]):
    ...
    async def get_permission(
        self, *, permission_id: int, db: AsyncSession
    ) -> Permission:
    ...
    async def get_permissions(
        self, *, skip: int = 0, limit: int = 100, db: AsyncSession
    ):
    ...
    async def update_permission(
        self, *, permission_id: int, obj_in: PermissionUpdate, db: AsyncSession
    ):
    ...
    async def assign_permission_to_role(
        self, *, permission_id: int, role_id: int, db: AsyncSession
    ):
        permission_in_db = await self.get_permission(db=db, permission_id=permission_id)
